...
(function () {
    ...
    function draw(t, animate) {
      ...
      for (var i = 0; i < cols; i++) {
        ...
        for (var j = 0; j < rows; j++) {
          ...
          var v = a01 * (0.4 + 0.2 * breathe) + boost;
          if (v < 0) v = 0;
          ...
          var hotState = hot > 0.4 ? (hot > 0.72 ? 2 : 1) : 0;
  ...
  function init() {
    ...
    var env = {
      ...
      density: " .:-=+*#%@",
      setFont: function (px) {
        ctx.font = px + 'px "IBM Plex Mono", ui-monospace, Menlo, monospace';
    ...
    // Full rate only while the pointer is actively shaping the field; the
    // idle ambient drift redraws at ~30fps, halving its paint cost.
    var lastPointerMove = -1e9;
    var lastDraw = -1e9;
    function loop() {
      if (!visible || reduce) {
        raf = null;
      ...
      var t = now() - t0;
      var interacting = now() - lastPointerMove < 1500;
      if (!interacting && t - lastDraw < 1000 / 30 - 1) return;
      lastDraw = t;
      elapsed = t;
      ctx.clearRect(0, 0, W, H);
      try {
    ...
    resize();
    try {
      ctrl = createEffect(ctx, env);
      if (ctrl.resize) ctrl.resize(W, H);
    } catch (e) {
      return;
    }
    ...
    window.addEventListener("mouseout", function () {
      if (ctrl && ctrl.pointer) {
        try {
          ctrl.pointer(0, 0, false);
        } catch (e) {}
      }
    });
    ...
    var hero =
      (canvas.closest && canvas.closest(".pl-hero")) || canvas.parentNode;
